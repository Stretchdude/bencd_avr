...
#define FIFO_write(fifo, data, size) {								\
	...
	if(tmphead != fifo._read) {				/* if buffer is not full */	\
		...
		/*fifo._write = tmphead; bens idee*/				/* store new index */		\
	}											\
}

#define FIFO_look(fifo, size) ( \
                 (FIFO_available(fifo)) ?                                        \
                 fifo._buffer[(fifo._write) & (size-1)] : 0      \
         )

#define FIFO_level(fifo, size) ( \
		((fifo._write & (size - 1)) - (fifo._read  & (size - 1))) \
		)
